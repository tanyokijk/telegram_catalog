...
class UpdateChatRequest extends FormRequest
{
    ...
    {
        $chatId = $this->route('chat'); // Assuming the route parameter is named 'chat'

        return [
            'chat_id' => 'required|integer|unique:chats,chat_id,'.$chatId,
            'user_id' => 'required|uuid|exists:users,id',
            'language_id' => 'required|uuid|exists:languages,id',
            'username' => 'required|string|max:32|unique:chats,username,'.$chatId,
            ...
            'meta_title' => 'required|string|max:128|unique:chats,meta_title,'.$chatId,
